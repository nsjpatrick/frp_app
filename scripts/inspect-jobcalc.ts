...
const wb = XLSX.readFile(FILE, { cellFormula: true, cellNF: true, cellStyles: false, sheetStubs: true });

const out: any = {
  sheetNames: wb.SheetNames,
  defined: (wb.Workbook?.Names ?? []).map((n: any) => ({ Name: n.Name, Ref: n.Ref, Sheet: n.Sheet })),
  sheets: {},
};
...
const outDir = path.resolve(ROOT, 'scripts', 'jobcalc-dump');
...
console.log('named ranges:', out.defined.length);
